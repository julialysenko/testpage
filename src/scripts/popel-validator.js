(function ($) {
    ...
    var pluginDefaults = {
        ...
        messageInjectElement: '.form-group',

        // Message inject method (called on message inject element)
        messageInjectMethod: 'append',
    ...
    Validator.prototype.validate = function () {
        ...
        // If validation is already in progress, return a rejected promise
        if (this.validating) return deferred.rejectWith(this).promise();
        ...
        this.$element.find(this.options.controlSelector).each(function () {
            promises.push(self.validateControl(this));
        });
        ...
        $.when.apply($, promises)
            .done(function () {
                ...
                if (self.options.disableSubmits) {
                    self.$element.find(self.options.submitSelector)
                        .prop('disabled', false)
    ...
    Validator.prototype.reset = function () {
        ...
        this.$element.find(this.options.controlSelector).each(function () {
            self.resetControlMessage(this);
            self.resetControlState(this);
        });
        ...
        if (this.options.disableSubmits) {
            this.$element.find(this.options.submitSelector)
                .prop('disabled', state < Validator.STATE_VALID)[
                    (state < Validator.STATE_VALID ? 'add' : 'remove') + 'Class'
    ...
    Validator.prototype.validateControl = function (control) {
        ...
        // If control is currently validating, return a rejected promise
        if (this.getControlState($control) === Validator.STATE_VALIDATING) return deferred.rejectWith(this).promise();
        ...
        $.each(rules, function (i, name) {
            var rule = self.rules[Validator.camelCase(name)];
            ...
            if (result.done && result.fail) {
                ...
                result.fail(function () {
                    promise.reject(self.getRuleMessage(name));
                });
            } else if (!result) {
                promise.reject(self.getRuleMessage(name));
        ...
        $.when.apply($, promises)
            .done(function () {
                self.resetControlMessage($control);

                self.setControlState($control, Validator.STATE_VALID);
            ...
            .fail(function (message) {
                ...
                self.setControlState($control, Validator.STATE_INVALID);
                $control.data('popel.validator.cache', {
                    value:   value,
                    promise: promise
                });

                self.setControlMessage($control, message);

                // Reject our promise
                deferred.rejectWith(self, [message]);
    ...
    Validator.prototype.getControlValue = function (control) {
        ...
        return $control.prop('type') === 'checkbox' || $control.prop('type') === 'radio'
             ? String($control.prop('checked'))
             : $control.val();
    };
    ...
    Validator.prototype.getControlState = function (control) {
        ...
        return $control.data('popel.validator.state') || Validator.STATE_NONE;
    };
    ...
    Validator.prototype.setControlMessage = function (control, message) {
        ...
        var text = message.replace(/:([^\s]+)/g, function (match, name) {
        ...
        if ($message.is(selector)) {
            ...
            $message.find(selector).text(text);
        }
    ...
    Plugin.setRuleMessage({
        ...
        checked:   'This checkbox must be ticked'
    ...
    $(document).on('blur.popel.validator change.popel.validator', _controlSelector, function () {
        ...
        // If control is currently validating, ignore the event
        if ($form.data('popel.validator').getControlState($control) !== Validator.STATE_VALIDATING) return true;
    ...
    $(document).on('keyup.popel.validator', _controlSelector, function (event) {
        ...
        tid = window.setTimeout(function () {
            ...
            // If control is currently validating, do nothing
            if ($form.data('popel.validator').getControlState($control) === Validator.STATE_VALIDATING) return;
    ...
    $(document).on('submit.popel.validator', _formSelector, function (event) {
        ...
        $form.data('popel.validator').validate().done(function () {
            var event = $.Event('resubmit.popel.validator');

            $form.trigger(event);

            if (_submit && !event.isDefaultPrevented()) {
                $form[0].submit();
            }
        });
    });
